...
static int backport_hci_sock_recvmsg(struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len, int flags)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,1,0)
static int backport_hci_sock_sendmsg(struct kiocb *iocb, struct socket *sock, struct msghdr *msg, size_t len)
{
